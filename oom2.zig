...
pub fn main() !void {
    ...
    var leaker: ?*u8 = try allocator.create(u8); //we have to leak since pointers are more than a byte

    std.debug.print("Allocating memory until a crash...\n", .{});
    while (true) {
        const newNode = allocator.create(Node) catch null;

        if(newNode==null){
            while (leaker!=null) {
                leaker = allocator.create(u8) catch null;
